...
Query Notify is a full-stack live-feed app. The frontend runs a Next.js/React dashboard with a compact tag-driven feed; the backend runs context feeds, state and national record lookups, RSS integrations, and document parsing.
...
- Keep context-feed normalization consistent: sources are generally shaped as `{ source, results }` and merged downstream.
...
- When changing UI state flows, keep tag selection, loading, and feed error handling centralized in `src/components/App.tsx`.
